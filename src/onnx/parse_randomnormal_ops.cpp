...
{
    const std::set<shape::type_t> valid_types = {shape::float_type, shape::half_type, shape::double_type}; 

    std::vector<op_desc> operators() const
    {
        return {{"RandomNormal"}, {"RandomNormalLike"}};
    }
                          ...
                          std::vector<instruction_ref> args) const
    {
        ...
        if (valid_types.find(out_type) == valid_types.end())
            MIGRAPHX_THROW(opd.op_name + ": invalid output type: " + std::to_string(dtype) + ". Valid types are 1 (float), 10 (half), and 11 (double).");
        ...
        else if(args.size() == 1)
        {
            if (valid_types.find(args[0]->get_shape().type()) == valid_types.end())
                use_dtype = true;
            out_shape = use_dtype ? shape{out_type, args[0]->get_shape().lens()} : args[0]->get_shape();
        }
        else 
        {
            MIGRAPHX_THROW(opd.op_name + ": cannot deduce shape without shape attribute or argument.");
        }
