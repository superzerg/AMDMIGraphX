...
TEST_CASE(dequantizelinear_test)
{
    ...
    auto r = mm->add_instruction(migraphx::make_op("dequantizelinear", {{"axis", 1}}), l0, l1, l2);
    mm->add_return({r});

    auto prog = migraphx::parse_onnx("dequantizelinear_test.onnx");
    EXPECT(p.sort() == prog.sort());
}

migraphx::program make_dequantizelinear_axis_prog(int axis)
{
    migraphx::program p;
...
migraphx::program make_quantizelinear_axis_prog(int axis)
{
    migraphx::program p;
    std::vector<size_t> input_lens{1, 1, 5, 1};
    auto* mm = p.get_main_module();
...
{
    migraphx::program p = make_quantizelinear_axis_prog(2);

    auto prog = migraphx::parse_onnx("quantizelinear_axis_test.onnx");
    EXPECT(p.sort() == prog.sort());
}
...
TEST_CASE(quantizelinear_neg_axis_test)
{
    ...
    auto prog = migraphx::parse_onnx("quantizelinear_neg_axis_test.onnx");
    EXPECT(p.sort() == prog.sort());
}
